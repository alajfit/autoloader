		...
		if ( rtAuto.num !== null && rtAuto.ele !== null && rtAuto.co !== null ) {
			...
			switch (rtAuto.co) {
				...
				case 'uk': // uk (complete)
					...
					break;
				case 'be': // belgium (In progress, working on international numbers)
					var $country = new RegExp("\\b(0)([1-9]{1})[0-9]{0,}(\\/| |)?[0-9]{2,3}( |\\.|)?[0-9]{2}( |\\.|)?[0-9]{2}\\b|\\b0([7-9]{1})[0-9]{1,}( |\\.|)?[0-9]{2,3}( |\\.|)?[0-9]{2,3}\\b","g");
					break;
				default:
					countryC = false;
					break;
